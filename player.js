...
if ( this.velocity.y > 0)
{
   if ((cell_down && !cell) || (cell_diag && !cell_right && nx) )
   {
    ...
    if ( (cell && !cell_down ) || (cell_right && !cell_dig && nx) )
  {
     this.position.y = tileToPixel(ty + 1);
	  ...
	  cell_down = getCellAtCoord(LAYER_PLATFORMS, tx, ty+2)
	  cell_diag = getCellAtCoord(LAYER_PLATFORMS, tx+1, ty+2)
	  ny = 0;
  }
}
if (this.velocity.x > 0)
{
    if ( (cell_right && !cell) || (cell_dig && !cell_down && ny) )
	{
	  this.position.x = tileToPixel(tx);
	  this.velocity.x = 0;
	}
}
else if ((cell && !cell_rigth) ||(cell_down && !cell_diag && ny) )
{
  if ((cell && !cell_rigth) || (cell_down && ! cell_diag && ny ))
  {
      this.position.x = tilToPixel(tx+1);
	  this.velocity.x = 0;
  }
}
